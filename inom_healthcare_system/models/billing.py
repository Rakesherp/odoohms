...
class OEBilling(models.Model):
    ...
    def _compute_balance(self):
        ...
        for rec in self:
            rec.balance = max(0.0, rec.patient_payable - rec.paid_amount)
        ...
        for rec in self:
            ...
            invoice = self.env['account.move'].create({
                ...
                'invoice_line_ids': [(0, 0, {
                    ...
                    'quantity': 1,
                    'price_unit': rec.patient_payable,
                })]
    ...
    def action_mark_paid(self):
        for rec in self:
            ...
            if rec.invoice_id.payment_state != 'paid':
                raise ValidationError(_('The hospital bill can be marked Paid only after the linked Odoo invoice is fully paid.'))
            rec.state = 'paid'
...
# ------------------------------------------------------------------
# Consultation-invoice support (used by oeh.appointment._create_
# consultation_invoice - see models/appointment.py). Appointments
# invoice directly via account.move (standard Odoo Accounting), not
# through oeh.billing, so these live as small inherits rather than
# on OEBilling itself.
# ------------------------------------------------------------------
