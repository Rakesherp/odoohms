...
class OEInsurance(models.Model):
    ...
    insurance_company = fields.Char(tracking=True, help='Legacy display field; linked insurance partner is preferred.')

    insurance_partner_id = fields.Many2one(
        'res.partner', string='Insurance Company', tracking=True, domain=[('is_company', '=', True)])
    ...
    remarks = fields.Text()
    ipd_id = fields.Many2one('oeh.ipd', string='IPD Admission', tracking=True, index=True)
    billing_id = fields.Many2one('oeh.billing', string='Hospital Bill', tracking=True, index=True)
    ...
    @api.constrains('claim_amount', 'approved_amount', 'settled_amount')
    ...
    def action_approve(self):
        for rec in self:
            if rec.claim_status not in ('submitted', 'under_review'):
            ...
            if rec.billing_id:
                rec.billing_id.insurance_claim_id = rec.id
                rec.billing_id.insurance_coverage = rec.approved_amount
    ...
    def action_settle(self):
        for rec in self:
            if rec.claim_status != 'approved':
                raise ValidationError(_('Only approved claims can be settled.'))
            amount = rec.settled_amount or rec.approved_amount
            if amount > rec.approved_amount:
                raise ValidationError(_('Settled amount cannot exceed approved amount.'))
            rec.write({
