...
class OEWard(models.Model):
    ...
    reserved_count = fields.Integer(compute='_compute_ward_stats')
    cleaning_count = fields.Integer(compute='_compute_ward_stats')
    maintenance_count = fields.Integer(compute='_compute_ward_stats')
    ...
    def _compute_ward_stats(self):
        for rec in self:
            ...
            rec.reserved_count = len(beds.filtered(lambda b: b.status == 'reserved'))
            rec.cleaning_count = len(beds.filtered(lambda b: b.status == 'cleaning'))
            rec.maintenance_count = len(beds.filtered(lambda b: b.status == 'maintenance'))
