from . import patient
from . import patient_allergy
from . import patient_history
from . import patient_visit
from . import vaccination

from . import doctor
from . import doctor_schedule
from . import appointment
from . import appointment_booking
from . import followup
from . import queue
from . import queue_board

from . import clinical_evaluation
from . import icd
from . import consent

from . import product_medicine      # NEW: extends product.product with clinical fields
from . import prescription_line
from . import prescription

from . import treatment
from . import medical_certificate

from . import laboratory
from . import sample_tracking
from . import radiology

from . import surgery
from . import ot_schedule
from . import surgical_team
from . import anesthesia_record

from . import medicine_batch

from . import pediatric
from . import therapy
from . import dental
from . import gynecology

from . import building
from . import ward
from . import bed
from . import ipd
from . import icu
from . import emergency
from . import nursing
from . import campus

from . import billing
from . import payment
from . import invoice
from . import insurance_claim
from . import hcfa_edi
from . import pre_authorization

from . import communication
from . import dashboard
from . import emr
from . import res_users

from . import doctor_dashboard
